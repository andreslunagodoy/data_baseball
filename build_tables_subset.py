...
df = pd.read_csv("raw_data/Batting.csv")
df = df[(df["yearID"] >= YEAR_MIN) & (df["AB"] > 0)]
df.to_csv("my_data/Batting.csv", index=False)
print(f"Batting: {len(df)} rows (yearID >= {YEAR_MIN}, AB > 0)")

# Filter Pitching to 2020+
df = pd.read_csv("raw_data/Pitching.csv")
df = df[df["yearID"] >= YEAR_MIN]
df.to_csv("my_data/Pitching.csv", index=False)
print(f"Pitching: {len(df)} rows (yearID >= {YEAR_MIN})")

# Filter Appearances to 2000+, only keep players in Batting or Pitching
batting = pd.read_csv("my_data/Batting.csv", usecols=["playerID", "yearID", "teamID"])
pitching = pd.read_csv("my_data/Pitching.csv", usecols=["playerID", "yearID", "teamID"])
valid_keys = pd.concat([batting, pitching]).drop_duplicates()
df = pd.read_csv("raw_data/Appearances.csv")
df = df[df["yearID"] >= YEAR_MIN]
df = df.merge(valid_keys, on=["playerID", "yearID", "teamID"])
df.to_csv("my_data/Appearances.csv", index=False)
